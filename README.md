...
Télécharger et lancer `murmur Setup x.x.x.exe` (voir [Compiler l'installeur](#compiler-linstalleur) si tu construis toi-même le paquet). L'installeur crée un raccourci sur le Bureau et dans le menu Démarrer. Le modèle `large-v3-turbo` est fourni directement ; le modèle `large-v3` (plus précis, plus lourd) se télécharge en option depuis les Réglages ou via `scripts/download-model.ps1`.
